...
exports.handler = async (event) => {
  ...
  if (!verification.valid || verification.payload?.purpose !== 'pflegegrad-return') {
  ...
  const paymentOrderRef = String(payment?.metadata?.orderRef || '').trim();
  const flowType = String(payment?.metadata?.flowType || '').trim();
  const paymentAmount = String(payment?.amount?.value || '').trim();
  const paymentCurrency = String(payment?.amount?.currency || '').trim();

  if (paymentOrderRef !== orderRef || flowType !== 'pflegegrad-startpaket') {
