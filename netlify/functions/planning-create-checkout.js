...
function createOrderRef() {
  return `planning-${Date.now()}-${crypto.randomBytes(6).toString('hex')}`;
}
...
exports.handler = async (event) => {
  ...
  if (PLANNING_PRICE === '0.00') {
    const paymentId = `free:versorgungsplanung:${orderRef}`;
    const freeReturnToken = createSignedToken({
      purpose: 'planning-return',
      orderRef,
      nonce,
      applicantName,
    ...
    return json(200, {
      ...
      orderRef,
      paymentId,
      checkoutUrl: `${baseUrl}/versorgungsplanung-abschluss.html?token=${encodeURIComponent(freeReturnToken)}`,
    });
  }
  ...
  const response = await fetch('https://api.mollie.com/v2/payments', {
    ...
    body: JSON.stringify({
      amount: {
        currency: 'EUR',
        value: PLANNING_PRICE,
      },
      description: `Versorgungsplanung für ${applicantName}`,
      redirectUrl: `${baseUrl}/versorgungsplanung-abschluss.html?token=${encodeURIComponent(returnToken)}`,
      metadata: {
        orderRef,
        flowType: 'versorgungsplanung',
        applicantName,
        customerEmail,
      },
    }),
